...
from third_party.CenterNet2.detectron2.layers import batched_nms
from third_party.CenterNet2.detectron2.modeling import ROI_HEADS_REGISTRY
from third_party.CenterNet2.detectron2.modeling.box_regression import Box2BoxTransform
from third_party.CenterNet2.detectron2.modeling.poolers import ROIPooler
from third_party.CenterNet2.detectron2.modeling.roi_heads import CascadeROIHeads
from third_party.CenterNet2.detectron2.modeling.roi_heads.cascade_rcnn import _ScaleGradient
from third_party.CenterNet2.detectron2.structures import Instances, Boxes, pairwise_iou
from third_party.CenterNet2.detectron2.utils.events import get_event_storage
from .cdkm_fast_rcnn import GRiTFastRCNNOutputLayers
...
from cdkm.data.custom_dataset_mapper import ObjDescription
...
class GRiTROIHeadsAndTextDecoder(CascadeROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None, task="ObjectDet"):
        ...
        if self.training:
            ...
            # RoI Head losses (For the proposal generator loss, please find it in cdkm.py)
            # 计算提议生成器损失（请在 cdkm.py 中找到）
...
class region_attention(nn.Module):
    ...
    def forward(self, in_feature):
        ...
        shortcut = x
        B, seq_len, _ = x.shape
        qkv = self.qkv(x).reshape(B, seq_len, 3, self.num_heads, -1).permute(2, 0, 3, 1,
                                                                             4)  # [3,B,self.num_heads,seq_len,head_dim]
